...
// prom-client, set in RdkafkaPrometheus constructor.
let prometheus;
...
class RdkafkaPrometheus {
    ...
    constructor(options = {}) {
        ...
        // Note that rdkafka classifies metrics as type 'counter' (or 'int'),
        // but effectively we cannot use prometheus.Counter here, as that only allows
        // incrementing (rightfully). We need a prometheus.Gauge, as we're just
        // reporting whatever rdkafka tells. At the same time all rdkafka 'gauge' metrics
        //  could be histograms for us, where we'd record the seen values over time.
        // This would lead to issues in having to define the buckets though,
        // and would make it harder to produce "current" statistics.
        // Abstract this into two functions so it can be looked at later.
        const makeRdKafkaCounter = (counterOptions) =>
            new prometheus.Gauge(Object.assign(counterOptions, { registers }));
        const makeRdkafkaGauge = (gaugeOptions) =>
            new prometheus.Gauge(Object.assign(gaugeOptions, { registers }));

        // Disable eslint from complaining about the order.
        // This is based on what rdkafka has in the documentation,
        // so make finding specific statistics faster.
        this.metrics = {
            // Top-level metrics
    ...
    _translateRdkafkaStat(key, value, labels, valueMapper = (v) => v) {
        ...
        if (metric) {
            try {
                ...
                // FIXME: prometheus.Counter doesn't have a "set value to",
                // but only a "inc value by".
        ...
        for (const key of Object.keys(brokerStats)) {
            ...
            case 'toppars':
                for (const topparName of Object.keys(brokerStats[key])) {
                    this._translateRdKafkaBrokerTopparsStats(
                        brokerStats[key][topparName], brokerLabels
                    );
                }
                break;
            case 'state':
                this._translateRdkafkaStat(
                    `broker_${key}`,
                    brokerStats[key],
                    brokerLabels,
                    (state) => {
                        ...
                        if (value === -1) {
                            logger.warn(
                                `Cannot map rdkafka broker state '${state}' to prometheus value`
                            );
                        }
                        return value;
                    }
    ...
    _translateRdkafkaTopicPartitionStats(statKey, topicPartitionStats, topicLabels) {
        ...
        for (const key of Object.keys(topicPartitionStats)) {
            ...
            case 'unknown':
                this._translateRdkafkaStat(
                    `topic_partition_${key}`,
                    topicPartitionStats[key],
            ...
            case 'fetch_state':
                this._translateRdkafkaStat(
                    ...
                    (state) => {
                        ...
                        if (value === -1) {
                            logger.warn(
                                'Cannot map rdkafka topic partition fetch ' +
                                `state '${state}' to prometheus value`
                            );
                        }
                        return value;
                    }
        ...
        for (const key of Object.keys(topicStats)) {
            ...
            case 'partitions':
                for (const topicPartitionId of Object.keys(topicStats[key])) {
                    this._translateRdkafkaTopicPartitionStats(
                        key, topicStats[key][topicPartitionId], topicLabels
                    );
                }
                break;
            default:
    ...
    _translateRdkafkaPrometheus(stats, extraLabels = {}) {
     ...
     *  keys must have already been declared when instantiating this RdkafkaPrometheus instance.
     * @return {void}
     */
    observe(stats, extraLabels = {}) {
        ...
        for (const labelKey in Object.keys(extraLabels)) {
            if (!allowedExtraLables.includes(labelKey)) {
                logger.warn(
                    `label '${labelKey}' was not declared in extraLabels when ` +
                    'RdkafkaPrometheus was instantiated, ignoring.'
                );
...
module.exports = RdkafkaPrometheus;
